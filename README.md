...
3. **Fill in the form**:
   ...
   - **Unsealing Private Key**: X25519 private key (non-standard PEM format) for unsealing secrets (will be encrypted with server's ingestion public key and stored)
     ...
     ```
     **Note**: Keys generated by `snpguard-client keygen` use a non-standard PEM format (raw 32-byte keys wrapped in PEM). This is NOT standard PKCS#8 format. Standard tools like `openssl` may not recognize this format.
