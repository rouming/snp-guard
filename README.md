...
8. **Service verifies** (in order):
   - Verifies nonce - must be generated by our code, signed with ephemeral secret, not expired (within +/- 60s window)
   - Verifies binding hash - SHA512(server_nonce || client_pub_bytes) must match report.report_data
