...
pub async fn verify_report_core(
    ...
    // Step 1: Verify nonce (must be generated by our code, signed with ephemeral secret, not expired)
    ...
    // Step 2: Verify binding hash (report_data must match SHA512(server_nonce || client_pub_bytes))
    ...
    // Verify VMPL (must be 0 for kernel level)
    ...
    // Unseal the VMK from sealed_blob using unsealing key
    if req.sealed_blob.len() < 32 {
        return AttestationResponse {
            success: false,
            encapped_key: vec![],
            ciphertext: vec![],
            error_message: "Client blob corrupted (too short)".to_string(),
        };
    }

    let (vmk_encapped_bytes, vmk_ciphertext) = req.sealed_blob.split_at(32);
    let unsealing_priv =
        match <X25519HkdfSha256 as Kem>::PrivateKey::from_bytes(&unsealing_priv_bytes) {
            ...
            Err(e) => {
                return AttestationResponse {
                    ...
                    encapped_key: vec![],
                    ciphertext: vec![],
                    error_message: format!("Invalid unsealing private key format: {}", e),
                };
            }
        };
        ...
        match <X25519HkdfSha256 as Kem>::EncappedKey::from_bytes(vmk_encapped_bytes) {
            ...
            Err(e) => {
                return AttestationResponse {
                    ...
                    encapped_key: vec![],
                    ciphertext: vec![],
                    error_message: format!("Failed to create VMK encapped key: {}", e),
                };
            }
        };
    ...
    let mut unsealing_ctx = match hpke::setup_receiver::<AesGcm256, HkdfSha256, X25519HkdfSha256>(
        ...
        Err(e) => {
            return AttestationResponse {
                ...
                encapped_key: vec![],
                ciphertext: vec![],
                error_message: format!("Failed to unseal VMK: {}", e),
            };
        }
    };
    ...
    let vmk_plaintext = match unsealing_ctx.open(vmk_ciphertext, &[]) {
        ...
        Err(e) => {
            return AttestationResponse {
                ...
                encapped_key: vec![],
                ciphertext: vec![],
                error_message: format!("Failed to decrypt VMK blob: {}", e),
            };
        }
    };

    // Reseal VMK for client session using client's ephemeral pub
    let client_pub = match <X25519HkdfSha256 as Kem>::PublicKey::from_bytes(&req.client_pub_bytes) {
        Ok(k) => k,
        Err(e) => {
            return AttestationResponse {
                success: false,
                encapped_key: vec![],
                ciphertext: vec![],
                error_message: format!("Invalid client public key: {}", e),
            };
        }
    };
    ...
    >(&OpModeS::Base, &client_pub, &[], &mut rng)
    {
        ...
        Err(e) => {
            return AttestationResponse {
                ...
                encapped_key: vec![],
                ciphertext: vec![],
                error_message: format!("Failed to setup session encryption: {}", e),
            };
        }
    };
    ...
    let ciphertext = match sender_ctx.seal(&vmk_plaintext, &[]) {
        ...
        Err(e) => {
            return AttestationResponse {
                ...
                encapped_key: vec![],
                ciphertext: vec![],
                error_message: format!("Failed to encrypt session response: {}", e),
            };
        }
    };
