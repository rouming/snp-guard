...
3. Verify nonce - must be generated by our code, signed with ephemeral secret, not expired (within +/- 60s window)
4. Verify binding hash - SHA512(server_nonce || client_pub_bytes) must match report.report_data (64 bytes)
5. Verify VMPL is 0 (kernel level)
6. Detect CPU family from report (cpuid fields parsed from AttestationReport)
7. Fetch AMD certificates (CA and VCEK) from AMD KDS using integrated `snpguest`
8. Verify certificate chain using integrated `snpguest`
9. Verify attestation report signature using integrated `snpguest`
10. Extract key digests:
   - ID_KEY_DIGEST at offset 0xE0 (48 bytes)
   - AUTHOR_KEY_DIGEST at offset 0x110 (48 bytes)
11. Look up attestation record by image_id + key digests, check policy flags/TCB minimums
12. Check if record is enabled
13. Decrypt unsealing private key from DB using ingestion private key (HPKE)
14. Unseal VMK from sealed_blob using unsealing private key (HPKE)
15. Reseal VMK for client session using client's ephemeral public key (HPKE)
16. Return success with encapped_key and ciphertext if all checks pass
