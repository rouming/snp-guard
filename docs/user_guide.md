...
**Important**: Keep this key secure! It will be encrypted and stored by the service using the master app key.
...
2. **Verify Key Digests**: The ID-Block and Auth-Block key digests must match those stored in the record (keys are generated by the server)
...
3. **Secure Key Storage**: 
   ...
   - The master app key (`/data/auth/master.app.key`) must be backed up securely
