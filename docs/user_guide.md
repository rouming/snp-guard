...
Before creating an attestation record, generate the unsealing keypair using the `snpguard-image` tool:
...
snpguard-image seal --pub-key unsealing.pub --data test.txt --out test.sealed

# Unseal with private key
snpguard-image unseal --priv-key unsealing.key --sealed-data test.sealed --out test.decrypted
...
```bash
# Using snpguard-image (recommended)
snpguard-image keygen --priv-out unsealing.key --pub-out unsealing.pub
```
...
**Key Format**: Keys generated by `snpguard-image keygen` use a non-standard PEM format (raw 32-byte keys wrapped in PEM). This is NOT standard PKCS#8 format. Standard tools like `openssl` may not recognize this format, but it works correctly with SnpGuard.
...
2. Fill in the form:
   ...
   - **Unsealing Private Key**: Paste the PEM-encoded private key (from `unsealing.key`, generated with `snpguard-image keygen`)
...
- `manage list|show|register|enable|disable|delete|export`: management calls using stored config by default. `manage show` prints kernel params and artifact filenames; `--json` is available for list/show.
- `manage register`: accepts `--staging-dir` (directory generated by `image convert --out-staging`) or individual `--firmware/--kernel/--initrd/--kernel-params` plus either `--unsealing-private-key` (plain key, will be encrypted) or `--enc-unsealing-private-key` (already encrypted key). `--unsealing-private-key` and `--enc-unsealing-private-key` are mutually exclusive. If `--staging-dir` is provided, it expects files: `firmware-code.fd`, `vmlinuz`, `initrd.img`, `kernel-params.txt`, and optionally `unsealing.key.enc`. Individual command options override files from `--staging-dir`. ID and auth keys are generated automatically by the server. Optionally accepts `--out-bundle` to export artifacts bundle after registration.

**Example (management export)**:
```bash
snpguard-client manage export --id <record-id> --format tar --out-bundle artifacts.tar.gz   # format: tar|squashfs
```
...
3. **Secure Key Storage**: 
   - ID-Block keys, Auth-Block keys, and unsealing private keys are all encrypted at rest using HPKE (Hybrid Public Key Encryption)
   - The ingestion private key (`/data/auth/ingestion.key`) must be backed up securely - if lost, encrypted keys cannot be recovered
   - ID-Block and Auth-Block keys are generated by the server, encrypted with the ingestion key, and stored in the database. Key files are deleted from the artifacts folder after block generation.
...
1. **Backup Ingestion Private Key**: Keep secure backups of `/data/auth/ingestion.key` - if lost, encrypted keys (ID, Auth, and unsealing) cannot be recovered
